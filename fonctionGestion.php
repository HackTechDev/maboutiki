...
function supprimerRepertoire($dir) {
    if (is_dir ($dir)) {
        $dh = opendir ($dir); 
    }else {     
        exit;
...
function supprimerBasedonnee($version, $sitedesc, $utilisateur, $motpasse, $sitenom, $prefixbd, $courriel, $nomcourriel){
	...
	$sql = "DROP DATABASE " . $sitenom .";";

	if ($conn->query($sql) === TRUE) {
	  echo "Base de donn&eacute;e " . $sitenom . " supprimer<br/>";
	} else {
	 echo "Erreur : " . $conn->error;
	}
...
function suppressionSite($version, $sitedesc, $utilisateur, $motpasse, $sitenom, $prefixbd, $courriel, $nomcourriel){
	// Permission du site : 777
	echo "Suppression site<br/>";
	
	supprimerBasedonnee($version, $sitedesc, $utilisateur, $motpasse, $sitenom, $prefixbd, $courriel, $nomcourriel);

	supprimerRepertoire("../sites/" . $sitenom);
	rmdir("../sites/" . $sitenom);	
}
